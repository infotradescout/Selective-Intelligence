...
- Let discovery recover the complete product, then bound execution to one end-to-end deliverable that can be built and proved in the current run. Preserve later deliverables; never make the person invent the phases.
...
Before building, creating, or finishing any product, feature, or system beyond a Tier 0 scratch throwaway, emit a locked full-scope intent checkpoint. The checkpoint recovers the largest truthful outcome the seed supports (the prompt is a seed, not the product-definition size), preserves the whole product as dependency-ordered deliverables with proof, and selects one active end-to-end deliverable that demonstrably fits the current execution window. Do not start until the whole-product definition is sufficient to prevent an incompatible foundation and the active deliverable is information-complete. Later non-dependent unknowns remain recorded for their own Definition Locks; they do not force an all-at-once build. Map canonical reuse, separate genuine authority decisions from inferred execution, reconcile conflicting constraints, choose the execution target from operational requirements, and enumerate the human-layer activation steps. Recover missing inputs by inference and never make the person design the phases. Scope erasure, permission-per-step, unbounded execution, layer-only slices, and partial starts on partial active-deliverable information are drift. This gate is unconditional: the checkpoint is the first move, not an option to skip. Read [references/first-checkpoint.md](references/first-checkpoint.md) and [references/execution-bounding-and-target-selection.md](references/execution-bounding-and-target-selection.md). The measure of this skill is how few corrections the person needs to reach a correct checkpoint; the target is zero.
...
- **Start:** Map a new project from intent through launch before coding. Define and lock the product, bounded first deliverable, scope boundaries, journeys, surfaces, architecture, canonical directories and ownership, database, APIs and integrations, UI/UX, security, operations, build order, execution target, acceptance gates, and change control. Preserve the complete product definition while executing only the active bounded deliverable, then reconcile it afterward. Read [references/start-mode.md](references/start-mode.md), [references/execution-bounding-and-target-selection.md](references/execution-bounding-and-target-selection.md), [references/actual-intent-alignment.md](references/actual-intent-alignment.md), [references/architecture-reuse.md](references/architecture-reuse.md), [references/ui-ux-and-output.md](references/ui-ux-and-output.md), and the risk-triggered [references/operational-safety-gates.md](references/operational-safety-gates.md) before creating project code.
...
Before any build-shaped mode, read [references/execution-bounding-and-target-selection.md](references/execution-bounding-and-target-selection.md) to separate full-product discovery from the active delivery boundary and to validate the execution target.
...
For every non-trivial new application, preserve the complete discovered product definition but make the first deliverable one bounded, end-to-end user loop with an observable ending and proof that fits the current run. Sequence the rest as later deliverables on the same architecture. Validate the boundary and target with `scripts/execution_contract.py`. Do not default to Sites for a production application whose core value depends on persistent operational data, complex permissions, substantial backend workflows, repository integration, business-state image processing, multi-stage business logic, migrations, audit history, or system-of-record behavior. Prefer the person's fitting established application or repository workflow. Sites remains suitable for bounded standalone experiences, prototypes, presentations, information surfaces, and simpler web products when it is genuinely the best target.
...
- Preserve the complete discovered product separately from the active execution scope. A bounded deliverable closes one loop; it never erases later deliverables or inflates itself into whole-product completion.
- Choose the execution target from the product's operating requirements. Do not use Sites as the default production foundation for operational applications with durable state, complex permissions, backend workflows, repository integration, or multi-stage business logic.
