...
For a Council-aware Start Pack, a positive completion claim is stronger than a checkpoint. `as-built=reconciled` and `release=closed` require the active build's registered `council-review.json` bridge to contain an already-applied, structurally valid Council pass. The bridge binds the exact project, release and version, build and lock, source revision, semantic baseline, validator, and active requirement set; the embedded case must carry the same non-null Start Pack binding. Each active requirement maps to a current valid proof and a structured runner receipt. Its exact JSON Pointer resolves to a passing receipt bound to the proof, requirement, source revision, procedure, expected and observed result, run, timestamp, and separately registered raw observation output; the validator recomputes both receipt and output digests from their bytes. Start Pack and Council governance prose may accompany proof but can never be its sole qualifying evidence. Missing, stale, substituted, self-referential, unverifiable, or merely plausible evidence blocks positive completion. Definition, Build, checkpoints, and partial, blocked, not-aligned, or unverifiable outcomes remain available because they do not claim successful completion.
