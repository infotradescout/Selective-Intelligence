...
A conforming client must demonstrate from fresh context that ordinary direct activation carries Orchestrator, Worker/Builder, and Objector responsibilities, with intent double-checking before Worker dispatch or work and result review before final claims. Material work challenges a plausible wrong reading and consequence; trivial self-contained checks remain lightweight. It uses separate bounded contexts for material project work when available and labels same-context checks degraded. JumpStart or Guided Council activates only when selected; extra Council roles need their own trigger. Safe Project-source promotion checks ownership, shared status, permitted data, and data-use settings; another AI service is optional.
