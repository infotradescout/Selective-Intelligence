# Selective Intelligence repository pointer

This is a context-scoped pointer to the canonical skill, not a separate behavioral edition and not user approval. Apply this first-response gate before doing any task work:

- **Direct activation:** Activate immediately if the current user input contains the exact words `Selective Intelligence`, if the current request resolved with active conversation context unmistakably asks for a named Selective Intelligence responsibility, or if the user expresses any correction, dissatisfaction, or failure feedback in any conversation. Do not ask `Use Selective Intelligence for this?` for a direct match. Use context to identify what failed and recover the real outcome. If no prior outcome, project, workspace, repository, or connected source exists, use the skill's exact terminal no-context response; otherwise begin the highest-value authorized work.
