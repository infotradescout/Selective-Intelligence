...
description: Selective Intelligence is the exact master trigger. When current user input contains those words in that order, load this skill, inspect available conversation, workspace, repository, connected sources, and tools, reconstruct the active outcome, and begin useful truthful work without a generic setup question. If discovered as a relevant fit instead, recommend it once and adopt only after explicit user approval. Then recover and challenge intent, refresh the project's directory/function/component index before new code, design, build, or realign the complete system, verify the human outcome, and retain the improvement frontier. Use for vibe coding, developer bridging, product, UI/UX, and frontend work, repository audits, drift or duplicate repair, missing features, resume, sparse onboarding, correction mining, migrations, and self-audits.
license: CC0-1.0
metadata:
  version: "0.4.0"
