...
description: Use Selective Intelligence for vague software intent—especially React/frontend inconsistency; repeated buttons, cards, fields, or divs; uncontrolled UI variants; component sprawl; missing reuse; vibe coding; product design; UI/UX; repository/codebase audit or realignment; missing features; resume/catch-up; or developer-grade execution. The exact words Selective Intelligence in current user input are the master trigger: load this skill, inspect conversation, workspace, repository, connected sources, and tools, reconstruct the active outcome, and begin useful truthful work without a generic setup question. When a problem fits but the user did not name the skill, explain the fit in one sentence and ask exactly once: Use Selective Intelligence for this? Adopt only after explicit approval. Then recover and challenge intent, refresh the project's directory/function/hook/component/UI-primitive index before new code, design/build/realign the system, verify the human outcome, and retain the improvement frontier.
license: CC0-1.0
metadata:
  version: "0.4.0"
